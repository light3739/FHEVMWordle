...
library ExternalEuint8Lib {
    function fromBytes(bytes memory b) internal pure returns (externalEuint8) {
        return abi.decode(b, (externalEuint8));
    }
}

contract FHEVMWordleMerkle is SepoliaConfig {
    using FHE for euint8;
    using ExternalEuint8Lib for bytes;
    ...
    mapping(address => bool) public playerPaused;
    ...
    modifier whenNotPaused() {
        require(!isEmergencyPaused, "Contract is emergency paused");
        require(!playerPaused[msg.sender], "Your game is paused");
        _;
    }

    modifier rateLimited() {
        require(block.timestamp >= lastGameStartTime[msg.sender] + MIN_GAME_INTERVAL, "Too frequent");
        _;
    }
    ...
    function setMerkleLeaves(uint32 _leaves) external onlyOwner {
        merkleLeaves = _leaves;
    }
    ...
    function setEncryptedSecretWord(
        ...
        require(msg.sender == player, "Can only set secret for yourself");
        require(!isEmergencyPaused, "Contract is emergency paused");

        require(MerkleProof.verify(merkleProof, merkleRoot, leaf), "Invalid merkle proof");
        require(encryptedLetters.length == WORD_LENGTH, "Incorrect number of letters");

        for (uint8 i = 0; i < WORD_LENGTH; i++) {
            externalEuint8 letter = encryptedLetters[i].fromBytes();
            encryptedSecretWords[player][i] = FHE.fromExternal(letter, inputProof);
    ...
    function startGame(bytes32 sessionHash) external whenNotPaused rateLimited {
        ...
        uint32 index = uint32(
            uint256(keccak256(abi.encodePacked(block.prevrandao, msg.sender, block.timestamp, totalGamesPlayed))) %
                merkleLeaves
        );
        g.wordIndex = index;
        g.secretSet = false;
    ...
    function requestDecryptResults() external whenNotPaused {
        GameData storage g = games[msg.sender];
        require(hasActiveGame[msg.sender], "No active game");
    ...
    function _completeGame(address pl, GameStatus fs) private {
        ...
        g.canRecover = false;
        activeGamesCount--;
        uint256 dur = g.endTime - g.startTime;
        bool win = (fs == GameStatus.Won);
        if (win) {
            s.gamesWon++;
            s.currentStreak++;
    ...
    function pauseMyGame() external {
        require(hasActiveGame[msg.sender], "No active game");
        playerPaused[msg.sender] = true;
        emit PlayerPaused(msg.sender);
    }

    function unpauseMyGame() external {
        playerPaused[msg.sender] = false;
        emit PlayerUnpaused(msg.sender);
    }
    ...
    function forfeitGame() external {
        require(hasActiveGame[msg.sender], "No active");
        _completeGame(msg.sender, GameStatus.Lost);
    }

    function isPlayerPaused(address player) external view returns (bool) {
        return playerPaused[player];
    }
}
